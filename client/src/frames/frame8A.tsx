...
export default function MintTRC20Tokens({ onBack }: MintTRC20TokensProps) {
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      if (receipt && receipt.result === 'SUCCESS') {
        if (receipt.log && receipt.log.length > 0) {
          ...
          alert(
            `Token successfully minted!\n\nToken Name: ${tokenName}\nToken Symbol: ${tokenSymbol}\nDecimals: ${DEFAULT_DECIMALS}\nInitial Supply: ${initialSupply}\nToken Address: ${newTokenAddress}\n\nYou can now add this custom token to your wallet.`
          )
        } else {
          alert(
            "Token minted successfully, but couldn't retrieve the token address. Please check your wallet for the new token."
          )
        }
      } else {
    ...
    setContractAddress('TRC20_CONTRACT_ADDRESS_HERE')
            ...
            {isTokenMinted && (
              <>
                <ContractAddressBox>
                  ...
                  <CopyButton onClick={handleCopyAddress} copied={copyButtonText === 'Copied!'}>
                    {copyButtonText}
                  </CopyButton>
                </ContractAddressBox>
                <HighlightedText>
