...
export default function SendTRX({ onBack }: SendTRXProps) {
  ...
  const handleSend = async () => {
    if (!recipient || !amount) {
      alert('Please enter both the recipient address and the amount of TRX to send.')
      return
    }

    // Simulating transaction
    const mockTxnHash = 'abc123def456ghijklmnop789'
    setTxnHash(mockTxnHash)
  ...
  const handleVerify = () => {
    if (verificationHash === txnHash) {
      alert('Transaction verified successfully!')
    } else {
      alert('Verification failed. Please check the transaction hash and try again.')
    }
  }
